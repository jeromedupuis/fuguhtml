<template>
  <div class="layout-clientArea_sidebar">

    <ul class="layout-clientArea_sidebarNav">
      <li v-for="(link, index) in links" :key="index" :class="[getLinkClass(link)]">
        <template v-if="link.sublinks && link.sublinks.length">
          <a class="layout-clientArea_sidebarNav_secondNavTrigger" :class="link.class" @click.stop="expandLink">
            <template v-if="link.icon">
              <img :src="link.icon" alt="" />
            </template>
            <span>{{ link.label }}</span>
          </a>
          <ul class="layout-clientArea_sidebarNav_secondNav">
      ...
      let links = [
        {
          ...
          to: '/client-area/projects'
        },
        {
          label: 'Resume',
          to: '/client-area/projects/'+this.projectId+'/resume',
      ...
      if(this.projectStatus == 'CLEARUP') {
        links = [
          ...links,
          {
            ...
            to: '/client-area/projects/'+this.projectId+'/clearup/details',
            icon: '/static/img/svg/010-creative.svg'
          }
        ];
        ...
        if(this.projectCategory == 'frontend') {
          links = [
            ...links,
            {
              ...
              to: '/client-area/projects/'+this.projectId+'/clearup/frontend',
              icon: '/static/img/svg/010-creative.svg'
            }
          ];
        }
        ...
        links = [
          ...links,
          {
            ...
            to: '/client-area/projects/'+this.projectId+'/clearup/tasks',
            icon: '/static/img/svg/010-creative.svg'
          }
        ];
      }
        ...
        links = [
          ...
          {
            label: 'Schedule',
            to: '/client-area/projects/'+this.projectId+'/schedule',
            icon: '/static/img/svg/001-clock.svg'
          },
          {
            label: 'Tasks',
            to: '/client-area/projects/'+this.projectId+'/tasks',
            icon: '/static/img/svg/013-pie-chart.svg'
          },
          {
            label: 'Review',
            to: '/client-area/projects/'+this.projectId+'/review',
            icon: '/static/img/svg/003-search.svg'
          },
          {
            label: 'Billing',
            to: '/client-area/projects/'+this.projectId+'/billing',
            icon: '/static/img/svg/015-banknote.svg'
          },
          {
            label: 'Download',
            to: '/client-area/projects/'+this.projectId+'/download',
            icon: '/static/img/svg/029-download.svg'
          },
          {
            label: 'Settings',
            to: '/client-area/projects/'+this.projectId+'/settings',
